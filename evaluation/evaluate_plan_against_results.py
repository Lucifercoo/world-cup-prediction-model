...
def evaluate(plan_path: Path) -> dict:
    ...
    plan = {match_key(row): row for row in read_csv(plan_path)}
    rows: list[dict] = []
    for result in results:
        key = match_key(result)
        if key not in plan:
            continue
        prediction = plan[key]
        actual_score = format_score(int(result["goals_a"]), int(result["goals_b"]))
        actual_outcome = score_outcome(*parse_score(actual_score))
        actual_bucket = total_goal_bucket(sum(parse_score(actual_score)))
        selected_bucket = prediction["adjusted_total_goal_bucket"]
        top2_buckets = parse_top2_total_goal_buckets(
        ...
        for column in SCORE_COLUMNS:
            ...
            any_score_bucket = any_score_bucket or total_goal_bucket(sum(parse_score(score))) == actual_bucket
            any_score_outcome = any_score_outcome or score_outcome(*parse_score(score)) == actual_outcome
        rows.append(
            {
                "actual_bucket": actual_bucket,
                "selected_bucket": selected_bucket,
                "outcome_hit": prediction["predicted_outcome"] == actual_outcome,
